...
class FaceBookBot():
    ...
    def parse_html(self, request_url):
        with requests.Session() as session:
            ...
            headers = {'user-agent':'Nokia6280/2.0 (03.60) Profile/MIDP-2.0 Configuration/CLDC-1.1'}
            post = session.post(self.login_basic_url, data=self.payload)
            parsed_html = session.get(request_url)
        return parsed_html

    def post_content(self):
        REQUEST_URL = f'https://mbasic.facebook.com/story.php?story_fbid={self.post_ID}&id=112704996810839'
